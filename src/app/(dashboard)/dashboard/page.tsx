...
export default function DashboardPage() {
  ...
  const depenses = txMois.filter((tx) => tx.type === 'SORTIE').reduce((s, tx) => s + tx.montant, 0)
            ...
            <ul className="space-y-2.5">
              {dernieresTransactions.map((tx) => (
                <li key={tx.id} className="flex items-center gap-3 group rounded-xl p-2 -mx-2 hover:bg-white/[0.03] transition-colors">
                  ...
                  <span className={`text-sm font-bold shrink-0 tabular-nums ${tx.type === 'ENTREE' ? 'text-emerald-400' : 'text-red-400'}`}>
