  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (budget) {
        ...
        toast({ title: 'Budget modifié', type: 'success' })
      } else {
        await budgetsApi.creer(payload)
        toast({ title: 'Budget créé', type: 'success' })
      }
      onSave()
      onClose()
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 mt-2">
          ...
          <DialogFooter className="mt-4 gap-2">
            ...
            <Button type="submit" disabled={loading}>
              {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : (budget ? 'Modifier' : 'Créer')}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
...
export default function BudgetsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast({ title: 'Budget supprimé', type: 'success' })
      refetch()
    } catch (err) {
      toast({ title: 'Erreur', description: err instanceof Error ? err.message : 'Erreur', type: 'error' })
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">{budgets?.length ?? 0} budget(s) actif(s)</p>
        </div>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={PieChart}
          title="Aucun budget"
          description="Créez des budgets pour contrôler vos dépenses"
          action={
            <Button onClick={() => { setSelected(null); setDialogOpen(true) }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4">
          {budgets.map((budget: Budget) => {
            ...
            const badgeLabel = pct >= 90 ? 'Dépassé' : pct >= 70 ? 'Attention' : 'OK'

            return (
              <div key={budget.id} className="rounded-xl border border-slate-700 bg-[#1E293B] p-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="flex justify-between text-xs text-slate-400 mt-1.5">
                    <span>{formatMontant(budget.montantDepense)} dépensé</span>
